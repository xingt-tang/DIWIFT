...
from utils.architecture import CL2X
...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        temperature = trial.suggest_categorical('temperature', [0.001, 0.01, 0.1])
        ...
        #     k = trial.suggest_categorical('k', [2])
        # elif self.model_pl.endswith('syn3/'):
        #     k = trial.suggest_categorical('k', [4])
        # else:
...
class Trainer(object):
    def __init__(self, epoch, model_pt, activation, mlp_dim, selector_dim, input_dim, k, temperature, bsize, lr,
        ...
        self.model = CL2X(model_pt=self.model_pt, selector_dims=self.selector_dims, layer_dims=self.layer_dims,
    ...
    def criterion(y, pred):
        ...
        loss = tf.reduce_mean(tf.reduce_sum(y_ * tf.math.log(pred_), axis=1))
        return loss
    ...
    def train_epoch(self, train_dataset, test_dataset):
        for ds in train_dataset.shuffle(self.bsize, reshuffle_each_iteration=True).batch(self.bsize):
            ...
            with tf.GradientTape() as tpe:
                y_pred, ori_y_pred, _ = self.model(x, self.k, self.temperature, is_training=True)
                loss = self.criterion(ori_y_pred, y_pred)
        ...
        for ds in test_dataset.batch(self.bsize):
            x, y = ds['x'], ds['y']
            pred, ori_pred, _ = self.model(x, self.k, is_training=False)
            #loss = self.criterion(ori_pred, pred)
...
def cl2x(tr_ds, val_ds, te_ds, model_pl='latents/synthetic_data_syn1/', iteration=300, seed=0, gpu_on=False,
         save_model=False, input_dim=11, n_trials=300, **unused):
    progress = WorkSplitter()

    progress.section("CL2X: Set the random seed")
    setup_seed(seed)

    progress.section("CL2X: Training")
    tuner = Tuner()
